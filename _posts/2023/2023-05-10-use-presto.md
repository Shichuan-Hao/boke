...
和 Presto 服务端一样，Presto CLI 也是通过 Maven 中央仓库分发二进制包。这个 CLI 应用程序以可执行 JAR 包的形式提供，可以像普通 UNIX 可执行程序一样使用它。

[在这里](https://repo.maven.apache.org/maven2/io/prestosql/presto-cli/)找到与我们使用的 Presto 服务端版本相一致的 Presto CLI 版本，而后下载对应的 `*-executable.jar` 文件，然后将其重命名为 presto。具体步骤如下。
...
在开始用 CLI 之前，需要先想好与哪个 Presto 服务端交互。在默认情况下，CLI 连接到运行在 `http://localhost:8080` 上的 Presto 服务端。如果 Presto 服务端在本地运行（出于测试或开发目的）或者通过 SSH 连接到了服务器，又或者使用 exec 命令连接到安装了 CLI 的 Docker 容器，则可以直接执行以下命令：
```shell
$ presto
presto >
```
...
1. 使用上下箭头滚动浏览
2. 使用 Ctrl-S 组合键和 Ctrl-R 组合键查询历史
