...
![](https://images.happymaya.cn/assert/structures/0401.gif)
...
![](https://images.happymaya.cn/assert/structures/0402.png)
...
![](https://images.happymaya.cn/assert/structures/0403.png)
...
- 对于一个单向链表，让最后一个元素的指针指向第一个元素，就得到了**循环链表**；
  ...
  ![](https://images.happymaya.cn/assert/structures/0404.png)

- 或者把结点的结构进行改造，除了有指向下一个结点的指针以外，再增加一个指向上一个结点的指针。这样就得到了**双向链表**；

  ![](https://images.happymaya.cn/assert/structures/0405.png)

- 还可以对双向链表和循环链表进行融合，就得到了**双向循环链表**，如下图所示：  

  ![](https://images.happymaya.cn/assert/structures/0406.png)
...
![](https://images.happymaya.cn/assert/structures/0407.png)
...
![](https://images.happymaya.cn/assert/structures/0408.png)
...
1. 按照位置序号来查找。
   ...
   ![](https://images.happymaya.cn/assert/structures/0409.gif)
...
2. 按照具体成绩查找。
   ...
   - 如果不是，则需要通过指针去判断下一个结点的值是否等于 95。以此类推，直到把所有结点都访问完。
     ...
     ![](https://images.happymaya.cn/assert/structures/0410.gif)

     ![](https://images.happymaya.cn/assert/structures/0411.gif)
...
2. 再新增一个数据结点。整体的复杂度就是 $O(n) + O(1)$。根据复杂度计算方法，这等同于 O(n) 的时间复杂度。
   ...
   ![](https://images.happymaya.cn/assert/structures/0412.gif)
...
2. 但对于单向链表，它的指针结构造成了自身数据通路有去无回，一旦修改某个指针，后面数据就会造成失联状态。为了解决这个问题，需要构造三个指针 privious、current  和 next，对当前结点、以及它之前和之后结点进行缓存，再完成翻转动作。
   ...
   ![](https://images.happymaya.cn/assert/structures/0413.gif)
...
- 快慢指针解法。该方法较为巧妙，其中快指针每次循环向后跳转两次，慢指针每次循环向后跳转一次。
  ...
  ![](https://images.happymaya.cn/assert/structures/0414.gif)
...
![](https://images.happymaya.cn/assert/structures/0416.png)
...
![](https://images.happymaya.cn/assert/structures/0415.gif)
