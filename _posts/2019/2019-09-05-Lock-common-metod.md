...
Lock 接口是 Java 5 引入的，最常见的实现类是 ReentrantLock，可以起到“锁”的作用。 锁是一种工具，用于控制对共享资源的访问，

Lock 和 synchronized 是两种最常见的锁，Lock 和 synchronized 都可以达到线程安全的目的，但是在使用上和功能上有很大的区别。所以 Lock 并不是用来代替 synchronized 的，而是当使用 synchronized 不合适或不足以满足要求的时候，Lock 可以用来提供更高级功能的。
