...
要想弄清楚存储器分级策略，需要搞清楚两点：

1. 弄清楚 ” 希望存储器是什么样子的 ”，也就是 "需求是什么"？
2. 弄清楚 ” 需求有哪些“实现约束 ”。

从需求上讲，希望存储器**速度快、体积小、空间大、能耗低、散热好、断电数据不丢失**。但在现实中，我们往往无法把所有需求都实现。
...
在大多数人的认知里，光速是很快的，而信号又是以光速传输的。既然光速这么快，那信号的延迟应该很小才对。但事实并不是这样，比如时钟信号是 1GHz 的 CPU，1G 代表 10 个亿，因此时钟信号的一个周期是 1/10 亿秒。而光的速度是 $3 ×10^8$ 次方米每秒，就是 3 亿米每秒。所以在一个周期内，光只能前进 30 厘米。

显然，虽然在宏观世界里光速非常快，但是到计算机世界里，光速并没有像认知中的那么快。所以即使元件离 CPU 的距离稍微远了一点，运行速度也会下降得非常明显。

干吗不把内存放到 CPU 里？如果这么做的话，除了整个电路散热和体积会出现问题，服务器也没有办法做定制内存了。也就是说 CPU 在出厂时就决定了它的内存大小，如果想换更大的内存，就要换 CPU，而组装定制化是你非常重要的诉求，这肯定是不能接受的。

此外，在相同价格下，一个存储器的速度越快，那么它的能耗通常越高。能耗越高，发热量越大。

因此，上面提到的需求是不可能被全部满足的，除非将来哪天存储技术有颠覆性的突破。
...
一种可行的方案，就是根据数据的使用频率使用不同的存储器：

- 高频使用的数据，读写越快越好，因此用最贵的材料，放到离 CPU 最近的位置；
- 使用频率越低的数据，放到离 CPU 越远的位置，用越便宜的材料。

![这是一张图片](https://maxpixelton.github.io/images/assert/os/0501.png)
...
![这是一张图片](https://maxpixelton.github.io/images/assert/os/0502.png)
...
比如 CPU 想访问一个内存地址，那么如何检查这个数据是否在 L1- 缓存中？换句话说，缓存中的数据结构和算法是怎样的？
...
之前我们学过，CPU 顺序执行内存中的指令，CPU 执行指令的速度是非常快的，一般是 2~6 个 CPU 时钟周期。
...
本文的内容，是所有缓存知识的源头。所有缓存系统的设计，都是存储资源的分级。

在设计缓存的时候，除了要关心整体架构外，还需要注意细节，比如：
- 条目怎么设计？
- 算法怎么设计？
- 命中率怎么统计？
- 缓存怎么置换等？